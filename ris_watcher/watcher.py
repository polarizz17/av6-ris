            ...
            def promote_when_stable(path: Path) -> None:
                try:
                    if not file_is_stable(path):
                        logger.debug(f"Not stable yet: {path.name}")
                        return
                    h = sha256_file(path)
                    if already_processed(h):
                        logger.info(f"Skip already-processed: {path.name}")
                        return
                    key = derive_group_key(path)
                    GROUPS.add_file(key, path)
                    logger.info(f"Added to group {key}: {path.name}")
                except Exception as e:
                    logger.warning(f"promote_when_stable error for {path}: {e}")

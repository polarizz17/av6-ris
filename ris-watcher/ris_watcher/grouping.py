...
class Group:
    ...
    def add(self, p: Path) -> None:
        self.files.add(p); self.last_seen = now()
class GroupManager:
    ...
    def add_file(self, key: str, p: Path) -> Group:
        with self._lock:
            ...
            if not g: g = Group(key); self._groups[key] = g
            g.add(p); return g
        ...
        with self._lock:
            ...
            for k, g in list(self._groups.items()):
                if g.last_seen < quiet_cutoff and g.files:
                    ready.append(g); self._groups.pop(k, None)
        cutoff = now() - timedelta(seconds=MAX_GROUP_AGE_SEC)
        too_old: List[Group] = []
        with self._lock:
            for k, g in list(self._groups.items()):
                if g.created_at < cutoff and g.files:
                    too_old.append(g); self._groups.pop(k, None)
        return ready + too_old
GROUPS = GroupManager()
def derive_group_key(p: Path) -> str:
    if HAVE_PYDICOM:
        try:
            ds = pydicom.dcmread(str(p), stop_before_pixels=True, force=True)  # type: ignore
            uid = getattr(ds, 'StudyInstanceUID', None)
            if uid: return f'study:{uid}'

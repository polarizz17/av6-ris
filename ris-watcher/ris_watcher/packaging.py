...
def build_zip_for_group(group_key: str, files: set[Path]) -> Path:
    ...
    with zipfile.ZipFile(tmp_zip, mode='w', compression=zipfile.ZIP_DEFLATED) as zf:
        for f in sorted(files):
            if not f.exists(): logger.warning(f'Skip missing: {f}'); continue
            try: rel = safe_rel_path(WATCH_DIR, f)
            except Exception: logger.warning(f'Skip outside watch dir: {f}'); continue
            zf.write(f, arcname=str(rel))
    tmp_zip.replace(final_zip); return final_zip
